...
class TemporalShift(nn.Module):
    def __init__(self, net, n_segment=3, n_div=8, inplace=False, offline=False):
    ...
    def forward(self, x):
        x = self.shift(x, self.n_segment, fold_div=self.fold_div, inplace=self.inplace, offline=self.offline)
        return self.net(x)

    @staticmethod
    def shift(x, n_segment, fold_div=3, inplace=False, offline=False):
        nt, c, h, w = x.size()
